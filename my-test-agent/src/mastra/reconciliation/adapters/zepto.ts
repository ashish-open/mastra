/**
 * Zepto Adapter — quick-commerce seller settlement reports.
 *
 * Upload-only. Zepto exports seller settlement CSVs from the partner portal.
 *
 ...
 *
 * Fee model:
 *   commission_rate   = 18% (varies by category in production)
 *   gst_on_commission = 18%
 *   tcs_rate          = 1%
 *   expected_net      = gross × 0.7776
 */

import type { SourceAdapter } from '../adapter.js';
import type { NormalizedTxn } from '../types.js';

export function zeptoExpectedNetPaise(grossPaise: number): number {
  const commission = grossPaise * 0.18;
...
function parseZeptoCSV(csv: string): NormalizedTxn[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const orderId = col(row, 'order_id') ?? `zepto_${i}`;
    ...
    txns.push({
      ...
      counterparty: 'Zepto',
      description: `Zepto net payout for order ${orderId}`,
      raw: { platform: 'zepto', grossPaise, commissionPaise, gstPaise, tcsPaise },
    });
  }
  return txns;
}

export const zeptoAdapter: SourceAdapter = {
  id: 'zepto',
  name: 'Zepto',
  kind: 'marketplace',
  async parseFile(file: Buffer, mime: string): Promise<NormalizedTxn[]> {
    if (!mime.includes('csv') && !mime.includes('text/plain')) {
      throw new Error(`Zepto adapter expects CSV; got '${mime}'`);
    }
    return parseZeptoCSV(file.toString('utf-8'));
  },
};
