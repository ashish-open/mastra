/**
 * Zomato Adapter — restaurant aggregator settlement reports.
 *
 * Upload-only (Zomato exports recon reports from the partner dashboard; no
 * public seller API).
 *
 ...
 *   tcs_rate          = 1%
 *   expected_net      = gross × 0.695
 */

import type { SourceAdapter } from '../adapter.js';
import type { NormalizedTxn } from '../types.js';

export function zomatoExpectedNetPaise(grossPaise: number): number {
  const commission = grossPaise * 0.25;
...
function parseZomatoCSV(csv: string): NormalizedTxn[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const orderId = col(row, 'order_id') ?? `zomato_${i}`;
    ...
    txns.push({
      ...
      counterparty: 'Zomato',
      description: `Zomato net payout for order ${orderId}`,
      raw: { platform: 'zomato', grossPaise, commissionPaise, gstPaise, tcsPaise },
    });
  }
  return txns;
}

export const zomatoAdapter: SourceAdapter = {
  id: 'zomato',
  name: 'Zomato',
  kind: 'marketplace',
  async parseFile(file: Buffer, mime: string): Promise<NormalizedTxn[]> {
    if (!mime.includes('csv') && !mime.includes('text/plain')) {
      throw new Error(`Zomato adapter expects CSV; got '${mime}'`);
    }
    return parseZomatoCSV(file.toString('utf-8'));
  },
};
